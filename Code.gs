// ================================================================
// ENJEKSİYON KONTROL — Google Apps Script v13 (TX_ID Biletleme + Canlı İzleme Onarımı)
// ================================================================
...
function doGet(e) {
  ...
  if (e.parameter.action === 'getStatus') {
    ...
    const vals = sheet.getRange(2, 1, lastRow - 1, 25).getValues(); // 25 sütun okuyoruz (TX ID dahil)
  ...
  if (e.parameter.action === 'submitForm') {
    ...
    const txId = e.parameter.tx_id || ''; 

    // MÜKERRER KAYIT KONTROLÜ (Aynı işlem bileti varsa yazma, atla!)
    if (txId) {
      const lastRow = sheet.getLastRow();
      if (lastRow > 1) {
        const checkRows = Math.min(lastRow - 1, 30); 
        const txVals = sheet.getRange(lastRow - checkRows + 1, 25, checkRows, 1).getValues(); // Y sütunu (25. Sütun)
        for (let i = 0; i < txVals.length; i++) {
          if (String(txVals[i][0]).trim() === String(txId).trim()) {
            return jsonp(cb, { result: 'ok', duplicate: true }); // Kayıt zaten var
          }
        }
      }
    }
    ...
    sheet.appendRow([
      ...
      txId // Y (25. Sütun - Bilet ID)
    ]);

    const adSoyad = e.parameter.adsoyad || '';
    updateCanliIzleme(
      e.parameter.enj1_no, e.parameter.kasa1, e.parameter.cevrim1, e.parameter.agirlik1,
      e.parameter.sayac_bas1 || '', e.parameter.sayac_bit1 || '', e.parameter.uretim1, e.parameter.fire1,
      vardiyaTarih, vardiya, adSoyad
    );
    if (enjSayisi === 2) {
      updateCanliIzleme(
        e.parameter.enj2_no, e.parameter.kasa2, e.parameter.cevrim2, e.parameter.agirlik2,
        e.parameter.sayac_bas2 || '', e.parameter.sayac_bit2 || '', e.parameter.uretim2, e.parameter.fire2,
        vardiyaTarih, vardiya, adSoyad
      );
    }
...
function yazBaslik(sheet) {
  const b = [
    ...
    'Onay', 'İşlem ID' // 25. Sütun Eklendi
  ];
  sheet.appendRow(b);
...
function updateCanliIzleme(enjNo, kasa, cevrim, agirlik, sayacBas, sayacBit, uretim, fire, tarih, vardiya, adSoyad) {
  ...
  // YENİ: Sekme yoksa veya başlık bozulduysa tabloyu sıfırdan yarat!
  if (!sheet) {
    sheet = ss.insertSheet('Canlı İzleme');
    _setupCanlıBaslik(sheet);
  } else if (String(sheet.getRange(1, 1).getValue()).trim() !== 'Makine') {
    _setupCanlıBaslik(sheet);
  }
  ...
  range.setValues([[
    'Enjeksiyon ' + enjIdx,
    adSoyad || '',
    tarih   || '', // Tek tırnak YOK (Sütun text formatlı olduğu için düzgün kalacak)
    kasa    || '',
    cevrim  || '',
    agirlik || '',
    sayacBas || '',
...
function _setupCanlıBaslik(sheet) {
  ...
  // Tarih sütununu metin formatında tut (Sheets bozmasın diye)
